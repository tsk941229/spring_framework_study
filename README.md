...
JPA는 다른 책 + 강의로 더 딥하게 공부 예정

JPA를 알기 전에 ORM (Object-Relation Mapping)을 먼저 알아야 한다  
ORM은 말 그대로 객체와 DB 테이블을 매핑해준다 (객체의 필드 = DB의 컬럼)  
그리고 SQL을 ORM 프레임워크가 각 DB회사에 맞춰 자동으로 만들어준다  
즉 ORM 프레임워크(JPA...)를 사용하면 더이상 SQL을 직접 코드에 작성하지 않아도 된다

이게 어떤 강점이 있냐면

1. 객체지향적인 개발이 가능하다
   - SQL에 맞추는게 아니라 도메인 모델(객체, 클래스) 중심으로 설계가 가능하다  


2. 유지보수성이 좋아진다
   - DB회사마다 SQL 문법이 다르기 때문에 코드에 SQL을 작성하면 (예를들어 mybatis xml) 처음 선택한 DB회사의 쿼리 문법에 종속되는데, ORM을 사용하면 DB회사의 문법에 종속되지 않는다 (JVM이 OS에 상관없는것 처럼)  
     Oracle로 개발하다가 Mysql로 바꾸고 싶으면 설정파일 (DataSource 또는 Driver 및 DB연동 설정 등..)만 바꿔주면 된다
   
단, 편리하고 좋은만큼 JPA를 제대로 쓰려면 학습할게 꽤 많다  
영속성 컨텍스트 (엔티티 생명주기, 1차 캐시, 변경 감지, 지연 로딩 fetch ... , 2차 캐시, 자주 나오는 N+1 문제 원인 및 해결 등..)  
JPA 문법 (JPQL, QueryDSL ...)  
트랜잭션과 동시성 (Lock 전략 등...)
